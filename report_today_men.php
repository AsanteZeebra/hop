...
<head>
  ...
  <title>Today's report | Welfare | Men</title>
...
<b style="font-size:30px">Mens Welfare Report</b> <br>
...
<br>
    ...
    <?php
                $today =  date("Y-M-jS"); 
                 ...
                 $sql = "SELECT * FROM dues WHERE status = 'Paid' AND department='Men' AND date_created='$today' ";
    ...
    <table class="table" style="float: Right; width:30%">
        ...
        <?php
                $today =  date("Y-M-jS"); 
                 ...
                 $sql = "SELECT SUM(amount) AS total FROM dues WHERE status = 'Paid' AND department='Men' AND date_created='$today' ";
                 $result = mysqli_query($con, $sql);
                 if ($result) {
                   while ($row = mysqli_fetch_array($result)) {
                   $amount = $row['total'];
